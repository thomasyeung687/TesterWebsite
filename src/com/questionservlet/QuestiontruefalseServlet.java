 ...
 */
@WebServlet("/QuestiontruefalseServlet")
public class QuestiontruefalseServlet extends HttpServlet {
		...
		if (request.getParameter("action").equals("submit") || request.getParameter("action").equals("another")) {
			try {
				...
				String query = "INSERT INTO questionsdatabase.allquestiontable(`idtest`, `tablename`) VALUES ('"
						+ request.getParameter("idtest") + "', 'questionsdatabase.truefalse');";
				...
				if(rSet.next()) { 
					...
					System.out.println("Failed to get LAST_INSERTED_ID TF"); 
				}
			...
			if (request.getParameter("action").equals("another")) {
				request.getRequestDispatcher("Questiontruefalse.jsp").forward(request, response);
			} else {
