        ...
        <div id="page-wrapper" >
        	<div id="page-inner">
                <div class="row">
					...
					<form action="TakeTest1By1Servlet" method="post">
					<div class="TestPageTestQuestionsDiv">
				  <% 
					  Question question = thisTest.getQuestionArray().get(currentQuestion); //no idea why this works
				  ...
				  %>
				  		...
				  		<div class="questionWrapper">
				  			<div>
					  			<div class="questionTextWrapper">
									<span style="white-space: pre-wrap; "><%out.print(currentQuestion+1); %>) <%out.print(question.getQuestion().trim()); %> </span> <!-- allows for space to be preserved for say if the question is a clump of code -->
								</div>
						...
						<%if(question instanceof MultipleChoiceQuestion){ 
							...
							for(String s : anschoices){
								...
								%> <input type="radio" name="answer" value="<%out.println(s); %>" required> <%out.println(s); %><br> <%
							}
							...
							%> 	<input type="radio" name="answer" value="True" required> True<br> 
								<input type="radio" name="answer" value="False" required> False<br> 
							<%
							...
							%> <textarea class="shortresponsequestion" name="answer"></textarea> <br> <%
							...
							for(String s: anschoices){
								...
								%> <input type="checkbox" name="answer" value="<%out.println(s); %>" > <%out.println(s); %><br> <%
							}
							...
							<input type="text" class="fiblank" name="answer">
							...
							for(int i = 0; i < strings.size()-1; i++){
								...
								%><input type="text" class="fiblank" name="<%=blank.get(i)%>"> <%
							}
							...
							for(Question questionComponent : questions){ //for each question in the multipart question, display it properly. questionComponent is basiscally a question in the multipart question
								String inputname = "answer"+questionComponentNumber++; //will be used as the name of the form input of the question in the request to retreive the ans choices to the question components in the multi question
								%>
								<div class="multipartQuestionComponentWrapper">
									<div>
							  			<div class="questionTextWrapper">
											<span style=" "><%out.print((currentQuestion+1)+"."+questionComponentNumber); %>) <%out.print(questionComponent.getQuestion().trim()); %> </span> <!-- allows for space to be preserved for say if the question is a clump of code -->
										</div>
										<div class="ptsWorthWrapper">
								...
								if(questionComponent instanceof MultipleChoiceQuestion){ 
									...
									for(String s : anschoices){
										//System.out.println(mc.getCorrectAns());
										%> <input type="radio" name="<%=inputname %>" value="<%out.println(s); %>" required> <%out.println(s);%><br> <%
									}
								}else if(questionComponent instanceof TFQuestion){
									TFQuestion tf = (TFQuestion) questionComponent;
									//System.out.println(tf.getAnswers().toString());
									%> 	<input type="radio" name="<%=inputname %>" value="True" required> True<br> 
										<input type="radio" name="<%=inputname %>" value="False" required> False<br> 
									<%
									
								}else if(questionComponent instanceof ShortResponseQuestion){
									ShortResponseQuestion sr = (ShortResponseQuestion) question;
									//do stuff if its a shortresponse.
									%> <textarea class="shortresponsequestion" name="<%=inputname %>"></textarea> <br> <%
									...
									for(String s: anschoices){
										//System.out.println(tf.getCorrectAns());
										%> <input type="checkbox" name="<%=inputname %>" value="<%out.println(s); %>" > <%out.println(s); %><br> <%
									}
									...
									List<String> correctans = fib.getCorrectans();
									out.println(fib.getStr1());%>
									<input type="text" class="fiblank" name="<%=inputname %>">
									<% out.println(fib.getStr2() +"<br>");
									...
									for(int i = 0; i < strings.size()-1; i++){
										out.println(strings.get(i));
										String inputnamefimb = inputname+blank.get(i);
										%><input type="text" class="fiblank" name="<%=inputnamefimb%>"> <%
									}
									if(fimb.isCasesensitive()){%>
									</br>
									<span>Case Sensitive: &#10004;</span>	
				  	...
				  	<%} %>
				  	<% if(thisTest.getAllowBackButton() && currentQuestion != 0){ %>
				  		<button name="action" value="prev" class="SShowTestbutton">Previous Question</button> 
				  	<%} %>
				  	<% if(thisTest.getQuestionArray().size()-1 != currentQuestion){ // if not last question %>
				  	<button name="action" value="next" class="SShowTestbutton">Next Question</button> 
				  	<%} %>
				  	<% if(thisTest.getQuestionArray().size()-1 == currentQuestion){ //if we are at the last question. Show Submit button %>
